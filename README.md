...
For definitions, formal problem statement, PELT, and a detailed toy-workflow description, see **[theoretical_background_cpd.md](theoretical_background_cpd.md)**.
...
├── xxi-ssdm-cmm-student-talk-cvergara-dds-uai.pdf
└── theoretical_background_cpd.md   # Theoretical companion (definitions, PELT, toy workflow)
```
...
2. Generate five figures in **`/img/`**:
...
- **AI-assisted development**: Parts of the code and documentation were improved with AI-based tools in **Cursor** (Cursor IDE, [cursor.com](https://cursor.com)).
 The use of PELT and the changepoint workflow was informed by the *Introduction to Changepoint Analysis* workshop (Rebecca Killick, CASI 2024): [IntroCptWorkshop.Rmd](https://github.com/rkillick/intro-changepoint-course/blob/master/IntroCptWorkshop.Rmd) (R `changepoint`, `changepoint.np`, PELT). The **binary segmentation** (adaptive-pooled) used in the piecewise model was developed by the author in the code, combining general ideas from the literature (Bai 1994; Fryzlewicz 2014).
- **PELT and changepoint methods**: The use of PELT and the changepoint workflow was informed by the *Introduction to Changepoint Analysis* workshop (Rebecca Killick, CASI 2024): [IntroCptWorkshop.Rmd](https://github.com/rkillick/intro-changepoint-course/blob/master/IntroCptWorkshop.Rmd) (R `changepoint`, `changepoint.np`, PELT). The **binary segmentation** (adaptive-pooled) used in the piecewise model was developed by the author in the code, combining general ideas from well-known literature (Bai 1994; Fryzlewicz 2014).
